...
df=read('../PI2/COVID-19_Reported_Patient_Impact_and_Hospital_Capacity_by_State_Timeseries.csv')
...
pto1 = df1[['state','inpatient_beds_used_covid']].groupby('state').agg({'inpatient_beds_used_covid':'sum'}).sort_values(by='inpatient_beds_used_covid').head(5)
pto1.reset_index(inplace=True)
